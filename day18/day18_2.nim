...
proc addLeft(n: Foo, x: int): Foo =
  ...
  of node: Foo(kind: node, l: n.l.addLeft(x), r: n.r).normalize()

proc addRight(n: Foo, x: int): Foo =
  case n.kind:
  of single: Foo(kind: single, v: n.v + x)
  of pair: Foo(kind: pair, a: n.a, b: n.b+x)
  of node: Foo(kind: node, l: n.l, r: n.r.addRight(x)).normalize()
...
proc main(inputFilename: string) =
  ...
  var ans = 0
  forProd i, j in input.bound, input.bound:
    if i == j: continue 
    let n = input[i] + input[j]
    let m = n.reduce()
    ans = max(ans, m.mag)
  echo ans
